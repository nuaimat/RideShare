...
public class CacheConnection {
    ...
    synchronized public static Connection checkOut(HttpServlet servlet) {
        String username = servlet.getServletContext().getInitParameter("db_username");
    ...
    synchronized public static Connection checkOut(String username, String password, String db, String host) {
        ...
        for (int i=0;!found && i<numberConnections;i++) {
            ...
            if (!cached.isInUse() && cached.getUrl().equals(Database.generateUrlString(host, db))) {
                if (verbose) {
                    System.out.println("found cached entry " +
                            Integer.toString(i) +
                            " for " + Database.generateUrlString(host, db));
                }
                found = true;
            }
        }
        if (found) {
            ...
            if (verbose) {
                ...
                System.out.println("Allocating new entry for " + Database.generateUrlString(host, db));
            }
            cached = new CachedConnection(
                    Database.getConnection(username, password, db, host), true, Database.generateUrlString(host, db));
