...
<div class=address1>
 User Name:${sessionScope.user.fullName} </div><div>
 Email: ${sessionScope.user.email}</div><div>
 State: ${sessionScope.user.state}</div><div>
 City: ${sessionScope.user.city}</div><div>
 Street: ${sessionScope.user.street}</div><div>
 Zip code: ${sessionScope.user.zipCode}</div><div>
 Bithday: ${sessionScope.user.birthYear}</div>
